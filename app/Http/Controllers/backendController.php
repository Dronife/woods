...
class backendController extends Controller
{
    ...
    public function userAdmin()
    {
        ...
        $pictureCount =  DB::table('pics')
                            ->select( DB::raw("COUNT(forest_id) as count"),
                                    'forest_id')
                            ->groupBy('forest_id')
                            ->get();  
        ...
        $info = [$types, $pictureCount];
        // dd($info[1]);
        // dd($info);
        // dd(($info[1]) );  
        //view('viewPhotos',['pics'=> $pics]);
        return view('admin', ['info' => $info, 'config'=>$config]);
    }
    ...
    public function deleteSubmitedForest($id)
   {
    ...
    public function updateSubmitedForest(Request $request)
    {
        ...
        ->update((['surname' => $request->surname,'lastname' => $request->lastname, 'email' => $request->email,
    ...
    public function getPictures($id){
        ...
        ->get();
        //dd($pics);
        return view('viewPhotos', ['pics' => $pics]);
    ...
    public function addForestConf(Request $request)
    {
        ...
        $values = [$request->surname, $request->lastname,$request->phone,$request->email,
